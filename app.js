...
function getSiteBasePath() {
    ...
    if (scriptPath.endsWith('/app.js')) {
        return scriptPath.slice(0, -'/app.js'.length);
    }
...
async function readJsonResponse(response) {
    ...
    try {
        ...
        console.error('Réponse non JSON reçue:', raw.slice(0, 500));
        throw new Error(response.ok
            ? 'Réponse serveur invalide.'
            : `Erreur serveur ${response.status}. Vérifiez le chemin API ou les logs PHP.`
        );
    }
}
...
function initialiserApplication() {
    ...
    initMenuToggle();
    initModals();
    initElegantContactForm();
}

function initElegantContactForm() {
    ...
    form.addEventListener('submit', async function (event) {
        ...
        if (!payload.nom || !payload.email || !payload.telephone || !payload.service || !payload.message) {
            showInlineFormMessage(messageBox, 'Veuillez remplir tous les champs du formulaire.', 'error');
            return;
        }
        ...
        showQuoteLoader('Envoi de votre demande de devis...');
        showInlineFormMessage(messageBox, '', 'success', false);

        try {
            const response = await fetch(getApiUrl('/app/api/submit_quote.php'), {
                method: 'POST',
            ...
            form.reset();
            showInlineFormMessage(messageBox, 'Votre demande de devis a bien été envoyée. Notre équipe vous répondra sous 24h.', 'success');
            afficherNotification('Demande de devis envoyée à ECOFI.', 'success');
        } catch (error) {
            console.error(error);
            showInlineFormMessage(messageBox, error.message, 'error');
        } finally {
            hideQuoteLoader();
            ...
            if (submitButton) {
                ...
                submitButton.innerHTML = submitButton.dataset.originalText || 'Envoyer ma demande';
            }
        }
    });
}

function showInlineFormMessage(element, message, type = 'success', visible = true) {
    element.className = `form-messages ${type}`;
    element.textContent = message;
    element.style.display = visible && message ? 'block' : 'none';
}
...
function initZoomFeature() {
    try {
        ...
        const imgContainer = document.getElementById('zoomImageContainer');
        ...
        let gallery = items.map(el => ({
            type: el.dataset.type || 'image',
            src: el.dataset.src || (el.querySelector('img') && el.querySelector('img').src) || '',
        ...
        function updateCounter() {
            if (zoomCounter) zoomCounter.textContent = `${currentIndex + 1} / ${gallery.length}`;
        }
        ...
        function openAt(index) {
            ...
            if (videoContainer) {
                ...
                // remove any iframe added
                const iframe = videoContainer.querySelector('iframe');
                if (iframe) iframe.remove();
                if (zoomVideo) {
                    zoomVideo.pause && zoomVideo.pause();
                    zoomVideo.removeAttribute('src');
                    const srcEl = zoomVideo.querySelector('source');
                    if (srcEl) srcEl.src = '';
                    zoomVideo.load && zoomVideo.load();
                }
            }

            if (item.type === 'image') {
                if (imgContainer && zoomImage) {
                    zoomImage.src = item.src;
                    imgContainer.style.display = '';
                }
            } else if (item.type === 'video') {
                // YouTube embed
                if (item.src && item.src.includes('youtube.com')) {
                    if (videoContainer) {
                    ...
                    if (zoomVideo && videoContainer) {
                        ...
                        if (srcEl) srcEl.src = item.src;
                            ...
                            // fallback: set src on video
                            zoomVideo.src = item.src;
                        }
                        zoomVideo.load && zoomVideo.load();
                        zoomVideo.play && zoomVideo.play().catch(() => {});
                        videoContainer.style.display = '';
                    }
                }
            }
            ...
            if (zoomDescription) zoomDescription.textContent = item.desc || '';
            updateCounter();
        ...
        function closeZoom() {
            const overlayEl = document.getElementById('zoomModalOverlay');
            if (!overlayEl) return;
            ...
            // remove any iframe in video container
            if (videoContainer) {
                const iframe = videoContainer.querySelector('iframe');
                if (iframe) iframe.remove();
                videoContainer.style.display = 'none';
        ...
        if (zoomPrev) zoomPrev.addEventListener('click', () => openAt(currentIndex - 1));
        if (zoomNext) zoomNext.addEventListener('click', () => openAt(currentIndex + 1));
        ...
        // update gallery variable if new items are added dynamically later
        window.openGalleryItemAt = openAt;
        window.closeGalleryZoom = closeZoom;
    } catch (err) {
        console.error('initZoomFeature error', err);
    }
}
